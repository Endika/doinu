/**
 * Minimal flat-dictionary i18n for the Doinu PWA (English + Spanish).
 *
 * Node-safe: no DOM access at import time. `resolveLocale` is pure (testable
 * without a browser); `createI18n` holds a mutable locale and a `t(key)` lookup.
 * Emojis live inside the strings so the UI keeps its playful labels in both
 * languages.
 */

export enum Locale {
  En = 'en',
  Es = 'es',
}

const MESSAGES = {
  en: {
    'menu.sub': "Let's play! 🎶",
    'act.melody': '🎵 Melody',
    'act.songs': '🎶 Songs',
    'act.import': '📂 Load MIDI',
    'act.practice': '🐢 Practice (wait)',
    'act.progress': '📊 Progress',
    'act.echo': '🔁 Echo',
    'act.memory': '🧠 Memory',
    'act.notefind': '🔎 Find the note',
    'act.ear': '👂 Listen & play',
    'act.rhythm': '🥁 Tap the beat',
    'act.scaleup': '🪜 Scale up',
    'act.scaleupdown': '🎢 Scale up & down',
    'act.free': '✨ Free play',
    'act.path': '🗺️ Path',
    'act.compose': '🎤 Make a melody',
    'nav.back': '← Menu',
    'songs.title': '🎶 Songs',
    'songs.pick': 'Pick a song 🎹',
    'songs.studies': '🎹 Scales & studies',
    'compose.title': '🎤 My melodies',
    'compose.sub': 'Make your own tune 🎹',
    'compose.recordNew': '● Record new',
    'compose.recording': 'Play your melody! 🎤',
    'compose.stop': '■ Stop',
    'compose.namePrompt': "What's your melody called?",
    'compose.save': '💾 Save',
    'compose.cancel': '✗ Cancel',
    'compose.empty': 'Record your first melody! 🎶',
    'compose.practice': '▶ Practice',
    'compose.delete': '🗑',
    'compose.defaultName': 'My melody',
    'path.title': '🗺️ Learning path',
    'path.sub': 'Learn step by step 🎹',
    'path.soon': 'Soon',
    'path.midiOnly': 'Needs a MIDI keyboard 🎹',
    'progress.title': '📊 Progress',
    'progress.sub': "How you're doing 🌟",
    'progress.empty': 'Play a little and your progress shows up here ✨',
    'hand.play': '▶ Play',
    'hand.right': '👉 Right',
    'hand.left': '👈 Left',
    'hand.both': '🙌 Both',
    'st.listen': 'Listen… 👂',
    'st.yourTurn': 'Your turn! 🎶',
    'st.watch': 'Watch… 👀',
    'st.find': 'Find this key! 🔎',
    'st.read': 'Read this note 🎼',
    'st.yes': 'Yes! ✅',
    'st.tryAgainEar': 'Try again 👂',
    'st.getReady': 'Get ready… 🥁',
    'st.freePlay': 'Free play — press any key',
    'st.midiError': 'Could not read that MIDI file 🙈',
    'st.noMidi': 'No MIDI keyboard access. Try the microphone instead.',
    'st.micTapToEnable': 'Tap "Enable microphone" to play 🎤',
    'st.micListening': 'Listening… play or sing a note! 🎤',
    'st.micDenied': 'Microphone blocked — allow it to play, or plug in a MIDI keyboard.',
    'st.saveFailed': "Couldn't save. Try again.",
    'input.micEnable': '🎤 Enable microphone',
    'input.useMic': '🎤 Use microphone',
    'input.useMidi': '🎹 Use MIDI keyboard',
    'wait.on': '🐢 Wait for the note: ON',
    'wait.off': '🐰 Wait for the note: OFF',
    'act.mictest': '🎤 Mic test',
    'mictest.title': '🎤 Mic test',
    'mictest.sub': 'See what the mic hears 🎧',
    'mictest.quiet': 'Quiet 🤫',
    'mictest.noise': 'Sound, no clear note 🌫️',
    'mictest.detecting': 'Note detected ✅',
    'mictest.sensitivity': 'Sensitivity',
    'mictest.calibrate': '🤫 Calibrate (2s quiet)',
    'mictest.calibrating': 'Stay quiet… 🤫',
    'st.level': 'Level',
    'st.best': 'Best',
    'st.tap': 'Tap!',
    'praise.amazing': 'Amazing!',
    'praise.great': 'Great!',
    'praise.good': 'Good try!',
    'praise.keep': 'Keep going!',
    'praise.again': "Let's try again! 🙈",
    'lang.toggle': 'ES',
  },
  es: {
    'menu.sub': '¡A jugar! 🎶',
    'act.melody': '🎵 Melodía',
    'act.songs': '🎶 Canciones',
    'act.import': '📂 Cargar MIDI',
    'act.practice': '🐢 Práctica (espera)',
    'act.progress': '📊 Progreso',
    'act.echo': '🔁 Eco',
    'act.memory': '🧠 Memoria',
    'act.notefind': '🔎 Encuentra la nota',
    'act.ear': '👂 Escucha y toca',
    'act.rhythm': '🥁 Sigue el ritmo',
    'act.scaleup': '🪜 Escala (sube)',
    'act.scaleupdown': '🎢 Escala (sube y baja)',
    'act.free': '✨ Juego libre',
    'act.path': '🗺️ Camino',
    'act.compose': '🎤 Crea tu melodía',
    'nav.back': '← Menú',
    'songs.title': '🎶 Canciones',
    'songs.pick': 'Elige una canción 🎹',
    'songs.studies': '🎹 Escalas y estudios',
    'compose.title': '🎤 Mis melodías',
    'compose.sub': 'Crea tu propia canción 🎹',
    'compose.recordNew': '● Grabar nueva',
    'compose.recording': '¡Toca tu melodía! 🎤',
    'compose.stop': '■ Parar',
    'compose.namePrompt': '¿Cómo se llama tu melodía?',
    'compose.save': '💾 Guardar',
    'compose.cancel': '✗ Cancelar',
    'compose.empty': '¡Graba tu primera melodía! 🎶',
    'compose.practice': '▶ Practicar',
    'compose.delete': '🗑',
    'compose.defaultName': 'Mi melodía',
    'path.title': '🗺️ El camino',
    'path.sub': 'Aprende paso a paso 🎹',
    'path.soon': 'Pronto',
    'path.midiOnly': 'Necesita teclado MIDI 🎹',
    'progress.title': '📊 Progreso',
    'progress.sub': 'Cómo lo llevas 🌟',
    'progress.empty': 'Juega un poco y tu progreso aparece aquí ✨',
    'hand.play': '▶ Tocar',
    'hand.right': '👉 Derecha',
    'hand.left': '👈 Izquierda',
    'hand.both': '🙌 Ambas',
    'st.listen': 'Escucha… 👂',
    'st.yourTurn': '¡Tu turno! 🎶',
    'st.watch': 'Mira… 👀',
    'st.find': '¡Encuentra esta tecla! 🔎',
    'st.read': 'Lee esta nota 🎼',
    'st.yes': '¡Sí! ✅',
    'st.tryAgainEar': 'Otra vez 👂',
    'st.getReady': 'Prepárate… 🥁',
    'st.freePlay': 'Juego libre — pulsa cualquier tecla',
    'st.midiError': 'No pude leer ese MIDI 🙈',
    'st.noMidi': 'Sin acceso al teclado MIDI. Prueba con el micrófono.',
    'st.micTapToEnable': 'Pulsa "Activar micrófono" para jugar 🎤',
    'st.micListening': 'Escuchando… ¡toca o canta una nota! 🎤',
    'st.micDenied': 'Micrófono bloqueado — permítelo para jugar, o conecta un teclado MIDI.',
    'st.saveFailed': 'No se pudo guardar. Inténtalo de nuevo.',
    'input.micEnable': '🎤 Activar micrófono',
    'input.useMic': '🎤 Usar micrófono',
    'input.useMidi': '🎹 Usar teclado MIDI',
    'wait.on': '🐢 Esperar la nota: SÍ',
    'wait.off': '🐰 Esperar la nota: NO',
    'act.mictest': '🎤 Probar micro',
    'mictest.title': '🎤 Probar micro',
    'mictest.sub': 'Mira qué oye el micro 🎧',
    'mictest.quiet': 'Silencio 🤫',
    'mictest.noise': 'Sonido, sin nota clara 🌫️',
    'mictest.detecting': 'Nota detectada ✅',
    'mictest.sensitivity': 'Sensibilidad',
    'mictest.calibrate': '🤫 Calibrar (2s silencio)',
    'mictest.calibrating': 'Quédate en silencio… 🤫',
    'st.level': 'Nivel',
    'st.best': 'Mejor',
    'st.tap': '¡Toca!',
    'praise.amazing': '¡Increíble!',
    'praise.great': '¡Genial!',
    'praise.good': '¡Bien!',
    'praise.keep': '¡Sigue!',
    'praise.again': '¡Inténtalo otra vez! 🙈',
    'lang.toggle': 'EN',
  },
} as const

export type MessageKey = keyof typeof MESSAGES.en

/**
 * Pure locale resolution. A valid stored override ('en' | 'es') wins; otherwise
 * a navigator language starting with 'es' selects Spanish, and everything else
 * (including an unknown stored value or absent navigator) falls back to English.
 */
export function resolveLocale(navLang: string | undefined, stored: string | null): Locale {
  if (stored === Locale.En || stored === Locale.Es) return stored
  if (navLang?.toLowerCase().startsWith('es')) return Locale.Es
  return Locale.En
}

export function createI18n(initial: Locale): {
  locale: Locale
  t: (key: MessageKey) => string
  set: (l: Locale) => void
} {
  const api = {
    locale: initial,
    t(key: MessageKey): string {
      return MESSAGES[api.locale][key] ?? MESSAGES.en[key]
    },
    set(l: Locale): void {
      api.locale = l
    },
  }
  return api
}
