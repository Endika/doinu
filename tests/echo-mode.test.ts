...
import { SequenceMatcher, ECHO_PHRASES, PressResult } from '../src/modes/echo-mode'

describe('SequenceMatcher', () => {
  it('matches a perfect echo with accuracy 1', () => {
    const m = new SequenceMatcher([60, 62, 64])
    expect(m.press(60)).toBe(PressResult.Correct)
    expect(m.press(62)).toBe(PressResult.Correct)
    expect(m.press(64)).toBe(PressResult.Correct)
    expect(m.done).toBe(true)
    expect(m.accuracy()).toBe(1)
  })
  it('counts wrong presses without advancing and lowers accuracy', () => {
    const m = new SequenceMatcher([60, 62])
    expect(m.press(61)).toBe(PressResult.Wrong) // wrong, no advance
