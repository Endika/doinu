...
describe('scale mode', () => {
  ...
  it('appends a descending tail without repeating the top note', () => {
    ...
    expect(chart.targets.map((t) => t.midi)).toEqual([
  ...
  it('passes/fails on the accuracy threshold', () => {
    ...
    expect(
      m.evaluate({ accuracy: 0.95, meanFindMs: 0, meanTimingDevMs: 0, tempoBpm: 60 }).passed,
    ).toBe(true)
    expect(
      m.evaluate({ accuracy: 0.4, meanFindMs: 0, meanTimingDevMs: 0, tempoBpm: 60 }).passed,
    ).toBe(false)
  })
