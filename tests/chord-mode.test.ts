...
describe('chord mode', () => {
  it('emits one co-timed target per note of each chord', () => {
    ...
    expect([...new Set(chart.targets.map((t) => t.startMs))].sort((a, b) => a - b)).toEqual([
      0, 1000, 2000,
    ])
  ...
  it('passes/fails on the accuracy threshold', () => {
    ...
    expect(
      m.evaluate({ accuracy: 0.7, meanFindMs: 0, meanTimingDevMs: 0, tempoBpm: 60 }).passed,
    ).toBe(true)
    expect(
      m.evaluate({ accuracy: 0.5, meanFindMs: 0, meanTimingDevMs: 0, tempoBpm: 60 }).passed,
    ).toBe(false)
  })
})
