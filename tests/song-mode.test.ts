...
describe('songHands', () => {
  it('offers R/L/both when a song has a left hand', () => {
    expect(songHands(twinkle)).toEqual<HandSelection[]>([
      HandSelection.Right,
      HandSelection.Left,
      HandSelection.Both,
    ])
...
describe('SongMode.evaluate', () => {
  it('passes at or above 80% accuracy', () => {
    ...
    expect(
      m.evaluate({ accuracy: 0.8, meanFindMs: 0, meanTimingDevMs: 0, tempoBpm: 0 }).passed,
    ).toBe(true)
    expect(
      m.evaluate({ accuracy: 0.5, meanFindMs: 0, meanTimingDevMs: 0, tempoBpm: 0 }).passed,
    ).toBe(false)
  })
})
